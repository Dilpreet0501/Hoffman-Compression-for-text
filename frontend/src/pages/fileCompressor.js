...
const FileCompressor = () => {
  ...
  const handleCompress = async () => {
    ...
    setIsLoading(true);
       await axios.post('https://file-compressor-backend-m3k1.onrender.com/compress', formData).then ((res)=>{
         const {success,downloadUrl}= res.data;

      if (success) {
        setDownloadUrl(`https://file-compressor-backend-m3k1.onrender.com${downloadUrl}`);
  ...
  const handleDecompress = async () => {
    ...
    formData.append('file', selectedFile);
    setIsLoading(true);
    await axios.post('https://file-compressor-backend-m3k1.onrender.com/decompress', formData).then((res)=>{
      const {success,downloadUrl}= res.data;
